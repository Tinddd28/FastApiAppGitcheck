...
async def check_user_exists(request: Annotated[requestModel, Depends()]) -> exist_user:
    ...
    if (list_):
        ...
        if int(data["num_in_list"]) > 0:
            return {"ok": True, "login": request.login, "num_in_list": int(data["num_in_list"]), "group_number": request.group_number, "status": "Good!"}
...
async def check_commit(request: Annotated[check_tests, Depends()]) -> return_info_lab:
    ...
    if (res[0] == -5):
        ...
        if res[1] == 1:
            if (flag):
                return {"ok": True, "number_of_lab": number_of_lab, "login": login, "group_number": request.group_number, "status": f"Тесты пройдены. Результат из-за просроченного дедлайна меньше на {res}!"}

            else:
                return {"ok": False, "number_of_lab": number_of_lab, "login": login, "group_number": request.group_number, "status": f"Тесты пройдены. результат из-за просроченного дедлайна меньше на {res}! Данные не были записаны"}
        else:
            if (flag):
                return {"ok": True, "number_of_lab": number_of_lab, "login": login, "group_number": request.group_number, "status": f"Тесты пройдены, половина баллов за один алгоритм снята!!! Результат из-за просроченного дедлайна меньше на {res}!"}

            else:
                return {"ok": False, "number_of_lab": number_of_lab, "login": login, "group_number": request.group_number, "status": f"Тесты пройдены, половина баллов за один алгоритм снята!!! Результат из-за просроченного дедлайна меньше на {res}! Данные не были записаны"}
