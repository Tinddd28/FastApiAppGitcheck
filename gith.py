...
def check_all_tests_passed(runs):
    taskid = 0
    for run in runs:
        run_id = run['id']
...
# Пример использования Укажите правильный SHA
workflow_runs = get_workflow_runs_by_sha(repo, sha)

if workflow_runs:
    taskid = check_all_tests_passed(workflow_runs)
...
def main(repo_name, sha, text_in_cfg):
